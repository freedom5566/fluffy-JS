...
exports.bookinstance_create_get = function (req, res, next) {
    ...
    Book.find({}, 'title')
        .exec(function (err, books) {
            if (err) { return next(err); }
            // Successful, so render.
            res.render('catalogView/create/bookinstance_form', { title: 'Create BookInstance', book_list: books });
        });
        ...
        if (!errors.isEmpty()) {
            ...
            Book.find({}, 'title')
                .exec(function (err, books) {
                    if (err) { return next(err); }
                    // Successful, so render.
                    res.render('catalogView/create/bookinstance_form', { title: 'Create BookInstance', book_list: books, selected_book: bookinstance.book._id, errors: errors.array(), bookinstance: bookinstance });
                });
            return;
        }
        else {
